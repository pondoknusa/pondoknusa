...
export function migrationsTableSql(grammar: SqlGrammar): string {
  ...
  switch (grammar.driver) {
    ...
    case 'oracle':
      ...
      END;`;
    case 'mssql':
      return `IF OBJECT_ID(N'migrations', N'U') IS NULL CREATE TABLE ${table} (
...
function hasTableQuery(
  ...
  switch (grammar.driver) {
    ...
    case 'oracle':
      return {
        sql: `SELECT table_name FROM user_tables WHERE table_name = ${grammar.parameter(1)}`,
        bindings: [tableName],
      };
    case 'mssql':
      return {
        sql: `SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_CATALOG = DB_NAME() AND TABLE_NAME = ${grammar.parameter(1)}`,
        bindings: [tableName],
      };
    default:
      return {
