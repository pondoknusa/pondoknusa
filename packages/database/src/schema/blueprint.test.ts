...
describe('Blueprint', () => {
  ...
  it('compiles oracle create table SQL', () => {
    const blueprint = new Blueprint('users', new OracleGrammar());
    blueprint.id();
    blueprint.string('email');
    blueprint.boolean('active');
    blueprint.timestamps();
    ...
    expect(blueprint.toCreateSql()).toBe(
      'CREATE TABLE "users" ("id" NUMBER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, "email" VARCHAR2(255) NOT NULL, "active" NUMBER(1) NOT NULL DEFAULT 0, "created_at" TIMESTAMP NOT NULL, "updated_at" TIMESTAMP NOT NULL)',
    );
  });

  it('compiles mssql create table SQL', () => {
    const blueprint = new Blueprint('users', new MssqlGrammar());
    blueprint.id();
    blueprint.string('email');
    blueprint.boolean('active');
    blueprint.text('bio');
    ...
    expect(blueprint.toCreateSql()).toBe(
      'CREATE TABLE [users] ([id] BIGINT IDENTITY(1,1) PRIMARY KEY, [email] NVARCHAR(255) NOT NULL, [active] BIT NOT NULL DEFAULT 0, [bio] NVARCHAR(MAX) NOT NULL, [created_at] DATETIME2 NOT NULL, [updated_at] DATETIME2 NOT NULL)',
    );
  });
  ...
  it('uses dialect-specific drop table SQL', () => {
    ...
    expect(new Blueprint('users', new OracleGrammar()).toDropSql()).toBe(
      'DROP TABLE IF EXISTS "users"',
    );
    expect(new Blueprint('users', new MssqlGrammar()).toDropSql()).toBe(
      'DROP TABLE IF EXISTS [users]',
    );
  });
});
